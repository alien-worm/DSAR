...
class BasicDataset(Dataset):
    ...
    def load_U_B_interaction(self):
        if self.task != 'train':
            ...
            with open(
                os.path.join(self.path, self.name, "{}.txt".format(self.prefix)),
                "r",
            ) as f:
                return list(
                    map(lambda s: tuple(int(i) for i in s[:-1].split("\t")), f.readlines())
...
class BundleTrainDataset(BasicDataset):
    def __init__(self, path, name, prefix=None):
        super().__init__(path, name, "train", 1, prefix=prefix)
        # U-B
...
def get_dataset(name, path="./data/", prefix=None):
    ...
    bundle_train_data = BundleTrainDataset(path, name, prefix=prefix)
