...
def write_log(run, log_path, topk, step, metrics, epoch):
    ...
    val_str = "%s, epoch_%d, Top_%d, Val:  recall: %f, ndcg: %f" % (
    curr_time, epoch, topk, val_scores["recall"][topk], val_scores["ndcg"][topk])
    test_str = "%s, epoch_%d, Top_%d, Test: recall: %f, ndcg: %f" % (
    curr_time, epoch, topk, test_scores["recall"][topk], test_scores["ndcg"][topk])
