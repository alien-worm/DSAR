...
class BasicDataset(Dataset):
    ...
    def __init__(self, path, name, task, neg_sample, prefix=None):
        self.path = path
        self.name = name
        self.task = task
...
class BundleTrainDataset(BasicDataset):
    def __init__(self, path, name, item_data, assist_data, seed=None, prefix=None):
        super().__init__(path, name, 'train', 1, prefix)
        # U-B
...
def get_dataset(path, name, task='tune', seed=123, prefix=None):
    ...
    bundle_train_data = BundleTrainDataset(path, name, item_data, assist_data, seed=seed, prefix=prefix)
    print('finish loading bundle {} data'.format(prefix))
