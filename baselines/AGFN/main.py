...
def main():
    ...
    for lr, l2_reg, embedding_size, num_layers, drop, leaky in \
            product(conf['lrs'], conf['l2_regs'], conf["embedding_sizes"], conf["num_layerss"], conf["drop"],
                    conf["leaky"]):
        ### Path
            ...
            for batch_i, batch in pbar:
                ...
                if epoch > conf["pre_epoch"] and (batch_anchor + 1) % test_interval_bs == 0:
                    ...
                    best_metrics, best_perform, best_epoch = log_metrics(conf, model, metrics, run, log_path
                                                                         ...
                                                                         best_perform, best_epoch, dataset_name)
                    ...
                    if dataset_name == 'Steam':
                        for topk in [5, 10, 15]:
                            ...
                            epoch, topk, metrics["val"]["recall"][topk], metrics["val"]["ndcg"][topk])
                            test_str = "%s, Top_%d, Test: recall: %f, ndcg: %f" % (
                            epoch, topk, metrics["test"]["recall"][topk], metrics["test"]["ndcg"][topk])
                            print(val_str)
                            print(test_str)
                    else:
