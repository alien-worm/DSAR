...
class PET(nn.Module):
    ...
    def ui_main_view_graph(self):
        ...
        item_level_graph = sp.bmat([[sp.csr_matrix((ubi_graph.shape[0], ubi_graph.shape[0])), ubi_graph],
                                    [ubi_graph.T, sp.csr_matrix((ubi_graph.shape[1], ubi_graph.shape[1]))]])
    ...
    def bi_main_view_graph(self):
        ...
        bundle_item_graph = sp.bmat([[sp.csr_matrix((bi_graph.shape[0], bi_graph.shape[0])), bi_graph],
    ...
    def propagate(self, test=False):
        ...
        if test:
            IL_users_feature, IL_items_feature = self.one_propagate(self.ui_main_graph_ori, self.users_feature,
                                                                    self.items_feature, self.ui_dropout, test)

        else:
            IL_users_feature_aug1, IL_items_feature_aug1 = self.one_propagate(self.ui_main_graph, self.users_feature,
                                                                              self.items_feature, self.ui_dropout, test)
            IL_users_feature_aug, IL_items_feature_aug = self.one_propagate(self.ui_main_graph_aug, self.users_feature,
                                                                            self.items_feature, self.ui_dropout, test)
        #  ============================= bundle level propagation =============================
        if test:
            BL_users_feature, BL_bundles_feature = self.one_propagate(self.ub_main_graph_ori, self.users_feature,
                                                                      self.bundles_feature, self.ub_dropout, test)
        else:
            BL_users_feature_aug1, BL_bundles_feature_aug1 = self.one_propagate(self.ub_main_graph, self.users_feature,
                                                                                self.bundles_feature, self.ub_dropout,
                                                                                test)
            BL_users_feature_aug, BL_bundles_feature_aug = self.one_propagate(self.ub_main_graph_aug,
                                                                              self.users_feature, self.bundles_feature,
                                                                              self.ub_dropout, test)

        # =========================== Bundle Item Propagation ================================
        if test:
            BI_bundles_feature, BI_items_feature = self.one_propagate(self.bi_main_graph_ori, self.bundles_feature,
                                                                      self.items_feature, self.bi_dropout, test)
        else:
            BI_bundles_feature_aug1, BI_items_feature_aug1 = self.one_propagate(self.bi_main_graph,
                                                                                self.bundles_feature,
                                                                                self.items_feature, self.bi_dropout,
                                                                                test)
            BI_bundles_feature_aug, BI_items_feature_aug = self.one_propagate(self.bi_main_graph_aug,
                                                                              self.bundles_feature, self.items_feature,
                                                                              self.bi_dropout, test)
        ...
        if test:
            ...
            bundles_feature = [BL_bundles_feature_aug1, BL_bundles_feature_aug, BI_bundles_feature_aug1,
                               BI_bundles_feature_aug]
    ...
    def cal_loss(self, users_feature, items_feature, bundles_feature, user_parameter):
        ...
        pred_pos = torch.mul(IL_user_parameter, IL_pred[:, 0]) + torch.mul(BL_user_parameter,
                                                                           BL_pred[:, 0]) + torch.mul(BI_user_parameter,
                                                                                                      BI_pred[:, 0])
        pred_neg = self.alpha * (torch.mul(IL_user_parameter, IL_pred[:, 1]) + torch.mul(BL_user_parameter,
                                                                                         BL_pred[:, 1]) + torch.mul(
            BI_user_parameter, BI_pred[:, 1]))
        ...
        bpr_loss_aux = (cal_bpr_loss(IL_pred) + cal_bpr_loss(BL_pred) + cal_bpr_loss(
            BI_pred)) / 3 + self.c_bpr * bpr_loss_item
    ...
    def forward(self, batch):
        ...
        self.users_feature_prop = [IL_users_feature_aug1, IL_users_feature_aug, BL_users_feature_aug1,
                                   BL_users_feature_aug, BI_users_feature_aug1]
        self.bundles_feature_prop = [IL_bundles_feature_aug1, BL_bundles_feature_aug1, BL_bundles_feature_aug,
                                     BI_bundles_feature_aug1, BI_bundles_feature_aug]
        self.items_feature_prop = [IL_items_feature_aug1, IL_items_feature_aug, BI_items_feature_aug1,
                                   BI_items_feature_aug]
        ...
        bpr_loss_main, bpr_loss_aux, c_loss, c_loss_int, up_reg = self.cal_loss(users_embedding, items_embedding,
                                                                                bundles_embedding, user_parameter)
