...
def get_cmd():
    ...
    parser.add_argument("-d", "--dataset", default="Youshu", type=str,
                        help="which dataset to use, options: iFashion, NetEase, iFashion")
    ...
    parser.add_argument("-p", "--prefix", default="", type=str, help="prefix for the train data")
...
def main():
    ...
    conf["prefix"] = paras["prefix"]
    conf["info"] = paras["prefix"]
    ...
    for lr, l2_reg, ui_dropout, ub_dropout, bi_dropout, embedding_size, num_layers, c_lambda, c_temp, c_lambda_int, c_temp_int, c_bpr, up_regs, c_aux, beta_bi, beta_ui, alpha in \
            product(conf['lrs'], conf['l2_regs'], conf['q_ui'], conf['q_ub'], conf['q_bi'], conf["embedding_sizes"],
                    conf["num_layerss"], conf["c_lambdas"], conf["c_temps"], conf["c_lambdas_int"], conf["c_temps_int"],
                    conf['c_bpr'], conf['up_regs'], conf['c_aux'], conf['beta_bi'], conf['beta_ui'], conf["alpha"]):
        ...
        for epoch in range(conf['epochs']):
            ...
            for batch_i, batch in pbar:
                ...
                loss = bpr_loss_main + conf['c_aux'] * bpr_loss_aux + conf['c_lambdas'] * c_loss + conf[
                    'c_lambdas_int'] * c_loss_int + conf["up_regs"] * up_reg
                ...
                pbar.set_description(
                    "epoch: %d, loss: %.4f, loss_bpr_main: %.4f, loss_bpr_aux: %.4f, c_loss: %.4f, c_loss_int: %.4f" % (
                    epoch, loss_scalar, bpr_loss_main_scalar, bpr_loss_aux_scalar, c_loss_scalar, c_loss_int_scalar))
        ...
        # save to json
        with open('{}.json'.format(paras['prefix']), 'w') as f:
...
def log_metrics(conf, model, metrics, run, log_path, checkpoint_model_path, checkpoint_conf_path, epoch, batch_anchor,
    ...
    if metrics["test"]["recall"][topk_] > best_metrics["test"]["recall"][topk_] and metrics["test"]["ndcg"][topk_] > \
            best_metrics["test"]["ndcg"][topk_]:
