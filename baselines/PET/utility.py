...
class BundleTrainDataset(Dataset):
    def __init__(self, conf, u_b_pairs, u_i_pairs, u_b_graph, u_i_graph, num_bundles, num_items, u_b_for_neg_sample,
                 u_i_for_neg_sample, b_b_for_neg_sample, i_i_for_neg_sample, name, neg_sample):
        self.conf = conf
        ...
        # with open(f'{self.conf["data_path"]}/{name}/{name}_ui.pkl', 'rb') as f:
        #     self.ui = pickle.load(f)
        with open(f'{self.conf["data_path"]}/{name}/user_item.txt', 'r') as f:
            self.ui = list(map(lambda s: tuple(int(i) for i in s[:-1].split('\t')), f.readlines()))
...
class Datasets():
    def __init__(self, conf):
        ...
        self.bundle_train_data = BundleTrainDataset(conf, u_b_pairs_train, u_i_pairs, u_b_graph_train, u_i_graph,
                                                    self.num_bundles, self.num_items, u_b_for_neg_sample,
                                                    u_i_for_neg_sample, b_b_for_neg_sample, i_i_for_neg_sample,
                                                    conf['dataset'], conf['neg_num'])
        ...
        self.item_train_data = ItemTrainDataset(conf, u_i_pairs, u_i_graph, self.num_items, u_i_for_neg_sample,
                                                i_i_for_neg_sample, conf['neg_num'])
    ...
    def get_ub(self, task):
        if task != 'train':
            ...
            with open(os.path.join(self.path, self.name, '{}.txt'.format(self.prefix)), 'r') as f:
                u_b_pairs = list(map(lambda s: tuple(int(i) for i in s[:-1].split('\t')), f.readlines()))
        ...
        print_statistics(u_b_graph, "U-B statistics in %s" % (task) + " " + self.prefix)
