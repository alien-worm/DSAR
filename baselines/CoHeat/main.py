...
@click.option('--prefix', type=str, default='user_bundle_train')
def main(seed, data, prefix):
    set_seed(seed)
    ...
    for epoch in range(1, conf["epochs"] + 1):
        ...
        if epoch % conf['test_interval'] == 0:
            ...
            all_metrics_result[epoch] = metrics['val']

    print('============================ BEST ============================')
    print(best_content)

    all_metrics_result['best'] = best_metrics
    with open('{}.json'.format(prefix), 'w') as f:
        json.dump(all_metrics_result, f)
