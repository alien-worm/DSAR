...
class Datasets():
    def __init__(self, conf):
        ...
        u_b_pairs_train, u_b_graph_train = self.get_ub("train", prefix=conf['prefix'])
    ...
    def get_ub(self, task, prefix=""):
        if task != 'train':
            ...
            with open(os.path.join(self.path, self.name, '{}.txt'.format(prefix)), 'r') as f:
                u_b_pairs = list(map(lambda s: tuple(int(i) for i in s[:-1].split('\t')), f.readlines()))
        ...
        print_statistics(u_b_graph, "U-B statistics in %s" %(task + prefix))
